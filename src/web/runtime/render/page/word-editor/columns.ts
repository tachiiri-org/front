  ...
  for (const node of list) {
    ...
    marker.addEventListener('click', (e: MouseEvent) => {
      ...
      if (state.activeDocNodeId === node.id) {
        ...
        document.dispatchEvent(new CustomEvent('word-editor:doc-toggle', {
          detail: { wordEditorFrameId: ctx.id, nodeId: null, nodeText: null },
        }));
      } else {
        state.activeDocNodeId = node.id;
        if (!state.docContentCache.has(node.id)) ctx.fetchDocContent(node.id);
        document.dispatchEvent(new CustomEvent('word-editor:doc-toggle', {
          detail: { wordEditorFrameId: ctx.id, nodeId: node.id, nodeText: node.text },
        }));
      }
      ctx.render();
      ctx.syncOuterWidth();
...
export const buildColumns = (ctx: WordEditorContext): HTMLElement => {
  const { state } = ctx;
