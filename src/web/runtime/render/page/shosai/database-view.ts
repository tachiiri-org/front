...
export function createDatabaseView(opts: {
  ...
  const { cellInput } = createCellInputs({
    onError: opts.onError, onOpenPage: opts.onOpenPage, onChanged: opts.onChanged,
  });
  ...
  const paint = (): void => {
    ...
    // 行を足す口は表の上にも置く。行が多いと、下まで送らないと足せないのは面倒。
    if (!detail.systemKind) {
      const top = el('button', { class: 's-add-row s-add-row-top', text: '＋ 行を追加' });
