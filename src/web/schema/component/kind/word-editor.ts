...
export const wordEditorDefaults: WordEditorComponent = {
  kind: 'word-editor',
  name: '',
  data: { nodes: [] },
};

export const wordEditorSchema: SchemaField[] = [
...
export const isWordEditorComponent = (value: unknown): value is WordEditorComponent => {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) return false;
  const c = value as Record<string, unknown>;
  if (c.kind !== 'word-editor') return false;
