...
export type DocumentEditorComponent = {
...
export const documentEditorDefaults: DocumentEditorComponent = {
  kind: 'document-editor',
  name: '',
  data: { nodes: [] },
};

export const documentEditorSchema: SchemaField[] = [
...
export const isDocumentEditorComponent = (value: unknown): value is DocumentEditorComponent => {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) return false;
  const c = value as Record<string, unknown>;
  if (c.kind !== 'document-editor') return false;
