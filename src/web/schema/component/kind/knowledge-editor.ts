...
export type KnowledgeEditorComponent = {
  kind: 'knowledge-editor';
  name?: string;
  data: TreeEditorData;
...
export const knowledgeEditorDefaults: KnowledgeEditorComponent = {
  kind: 'knowledge-editor',
  name: '',
  data: { nodes: [] },
};

export const knowledgeEditorSchema: SchemaField[] = [
...
export const isKnowledgeEditorComponent = (value: unknown): value is KnowledgeEditorComponent => {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) return false;
  const c = value as Record<string, unknown>;
  if (c.kind !== 'knowledge-editor') return false;
