...
export type GridCanvasComponent = LegacyGridCanvasComponent | ModernGridCanvasComponent;

export type ModernGridCanvasComponent = {
  kind: 'grid-canvas';
...
type LegacyGridCanvasComponent = {
  kind: 'grid';
  targetComponentId: string;
  viewportWidth?: number;
  viewportHeight?: number;
...
export const gridCanvasDefaults: ModernGridCanvasComponent = { kind: 'grid-canvas' };
...
export const isGridCanvasComponent = (value: unknown): value is GridCanvasComponent => {
  ...
  if (c.kind === 'grid') {
    return (
      typeof c.targetComponentId === 'string' &&
